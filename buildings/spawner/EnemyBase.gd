...
func spawn_enemys():
	...
	var level_cor = map.levels[ebene-1]
		
	for e in enemys:
		e.speed = e.speed * rand_range(0.9, 1.1)
		e.max_health = e.max_health * (1 + ebene * 0.1)
		e.add_spawner(self)
		var effects = map.get_enemy_effects(map.level_cor_to_id(level_cor))
		for eff in effects:
...
func remove_enemy(var _enemy):
	...
	if enemys.empty() and buliding_on_death:
		call_deferred("emit_signal", 'Spawn_Building', buliding_on_death, cor)
		queue_free()
